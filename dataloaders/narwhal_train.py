...
class NarwhalTrain(data.Dataset):
    def __init__(
        self,
        split='val',
        root=Path.db_root_dir('narwhal_train'),
        transform=None,
        area_thres=0,
        retname=True,
        use_thin=False):
        ...
        if self.use_thin:
            self.thins_dir = os.path.join(Path.db_root_dir('thin_regions'), 
                'narwhal_train_' + self.split, 'gt_thin')
        ...
        if (not self._check_preprocess()):
            print('Pre-processing NarwhalTrain dataset, this will take long, but it '
    ...
    def _make_img_gt_pair(self, index):
        ...
        _img = _img[:, :, np.newaxis] # Carl added: Make new axis
        _img = np.repeat(_img, 3, axis=2) # Carl added: Made greyscale -> RGB
...
if __name__ == '__main__':
    ...
    dataset = NarwhalTrain(split='train', transform=transform, use_thin=True)
