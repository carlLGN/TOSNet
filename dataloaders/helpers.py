...
def find_points_inbetween(p1, p2, points_to_sample=1):
    """ Sample points equally spaced on a line between two points
    """
    points = np.empty((points_to_sample,2))
    t = 0
    for i in range(points_to_sample):
        t+=1/(points_to_sample+1)
        cx = np.round(p1[0]*(1-t)+p2[0]*t)
        cy = np.round(p1[1]*(1-t)+p2[1]*t)
        points[i] = np.array([cx, cy])
    return points

def domain_specific_gaussian_transform(img, labels, sigma=10):
    """ Perform gaussian transformation using distance transformation.
    Args:
        img: The original color image
        labels: Label with the Gaussian center(s) [[x0, y0],[x1, y1],...]
    ...
    if labels is None:
        ...
        labels_sorted = labels[np.argsort(labels[:,1])]
        
        top_bottom_labels = np.array([labels_sorted[0,:], labels_sorted[3,:]])
        inbetween_labels = find_points_inbetween(top_bottom_labels[0], top_bottom_labels[1], points_to_sample=points_to_sample)
        labels = np.vstack((top_bottom_labels, inbetween_labels)).astype(np.int32)
